...
let state = "idle"; // "coocing" - готовка, "ready"-кофе готов /переменная отвечает за текущее состояние кофе-машины
...
function takeMoney(event, bill) {
  event.preventDefault();// приостанавливает стандартное событие браузера
...
function inAtm(bill) {
  ...
  // console.log([atmLeftTopX, atmLeftTopY, atmLeftBottomX, atmLeftBottomY, billLeftTopX, atmRightTopY, billLeftTopX, billLeftTopY, billRightTopX, billRightTopY]);
...
function createCoin(nominal) {
  ...
  coin.style.transition = "transform 300ms easy-in";
